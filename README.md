...
Python package for fitting a set of candidate curves to 1D sampled data and retrieving the position and width of a single peak or valley.
...
- Peak coordinates in normalized and original data units;
...
These commands work in Windows CMD, PowerShell and common Unix shells. Run them with the intended Python environment active.
...
The editable installation (`-e`) lets Python use the source files directly. The development tools are pytest for tests, Ruff for code checks and mypy for type checks.
...
# Supply either include_funcs or exclude_funcs, not both.
```
...
- `"IC"`: corrected Akaike information criterion (AICc), which balances fitting error against the number of parameters.

All candidates are fitted using least squares, including when MAE is used for ranking. Peak/valley variants and starting guesses are compared by RMSE within each model. Available criteria can be checked through `fitter.best_fit_criteria`; AICc availability is updated for the selected candidates. An unavailable criterion falls back to RMSE with a warning.

`fit.pcov` is the parameter covariance estimate and `fit.perr` contains its diagonal square roots. These are approximate parameter uncertainties, not uncertainty intervals for the peak position or FWHM. Polynomial fits currently return `None` for both fields.

### Input requirements and limitations

- Provide equally sized, finite, real arrays with at least two samples. One-column arrays are also accepted. X values must be unique; X and Y must both be nonconstant. X is sorted together with its corresponding Y values.
...
- `filter_spikes=True` removes eligible peaks with fewer than three nearby samples, or with a sufficiently high ratio of local to total RMSE. `filter_line_fit=True` performs its additional comparison only when a line has not already been fitted. These filters are heuristics, not statistical significance tests.
