...
from peakfitpy.utils.fitting_funcs import default_f_params, gaussian_f
...
def test_basic_fitting():
    ...
    for noise in noise_fractions:
        ...
        if is_peak is not None:
            assert is_peak and 0.4 < xp < 0.6 and 7.5 < yp < 12.5, f"Peak fitted not consistent: {is_peak, xp, yp}"
